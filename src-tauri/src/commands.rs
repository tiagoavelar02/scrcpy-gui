...
fn build_scrcpy_args(config: &ScrcpyConfig, video_dir_fallback: Option<String>) -> Vec<String> {
    ...
    if config.session_mode == "mirror" && (hid_keyboard || hid_mouse) && otg_pure {
        ...
        if let Some(audio) = config.audio_enabled {
            if !audio {
                args.push("--no-audio".to_string());
            }
        }
...
pub async fn download_scrcpy(window: Window) -> Result<(), String> {
    ...
    let temp_archive_path_clone = temp_archive_path.clone();
    let temp_extract_dir_clone = temp_extract_dir.clone();
    let extension_clone = extension.to_string();

    tokio::task::spawn_blocking(move || {
        if extension_clone == ".zip" {
            let file = std::fs::File::open(&temp_archive_path_clone)
                .map_err(|e| format!("Failed to open zip: {}", e))?;
            let mut archive = zip::ZipArchive::new(file)
                .map_err(|e| format!("Failed to read zip archive: {}", e))?;
            archive
                .extract(&temp_extract_dir_clone)
    ...
    let mut entries = tokio::fs::read_dir(&temp_extract_dir)
        .await
        .map_err(|e| e.to_string())?;
    if let Some(entry) = entries.next_entry().await.map_err(|e| e.to_string())? {
        ...
        if path.is_dir() {
            ...
            tokio::task::spawn_blocking(move || {
                let _ = std::fs::rename(&path_clone, &extract_path_clone)
                    .or_else(|_| copy_dir_all(&path_clone, &extract_path_clone));
            })
            .await
            .map_err(|e| e.to_string())?;
        } else {
            ...
            let extract_path_clone = extract_path.clone();
            tokio::task::spawn_blocking(move || {
                let _ = std::fs::rename(&temp_extract_dir_clone, &extract_path_clone)
                    .or_else(|_| copy_dir_all(&temp_extract_dir_clone, &extract_path_clone));
            })
            .await
            .map_err(|e| e.to_string())?;
        }
    ...
    if tokio::fs::try_exists(&temp_archive_path)
        .await
        .unwrap_or(false)
    {
